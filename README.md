...
Super Smash Bros Melee Vision (SSBMV) is a computer vision pipeline designed to detect, track, and match characters in Super Smash Bros Melee gameplay footage using classical image processing techniques built using OpenCV. This pipeline is designed to run on recorded footage and produces a game state that can be streamed to stdout or written to a json-lines (.jsonl) file. Please see the accompanying paper for more information located at [/paper/ssmbmv.pdf](./paper/)
...
**NOTE:** This project relies on pre-compiled templates for actor matching which are located in the `/data/templates/` directory.
...
Run the SSBMV video analysis pipeline.
...
| `--index` |  | No | `str` | `./index.json` | File path (`.json`) | Path to the template index generated by the `init` command. |
...
This will run all test cases for all features across each stage and aggregate results into a `test_results/` folder. This automatically runs the analysis script described in [Analyzing Results](#analyzing-results) and requires the ground truth data files to exist in `./tests/labels`.
